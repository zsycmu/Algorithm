...
public class _517_Super_Washing_Machines {
	public int findMinMoves(int[] machines) {
        ...
        if (sum[sum.length - 1] % machines.length != 0) {
            return -1;
        }
        
        int avg = sum[sum.length - 1] / machines.length;
        int result = 0;
        
        for (int i = 0; i < machines.length; i++) {
            int left = sum[i] - avg * i;
            int right = sum[sum.length - 1] - sum[i + 1] - avg * (machines.length - i - 1);
            
            if (left > 0 && right > 0) {
                result = Math.max(result, Math.max(left, right));
            } else if (left < 0 && right < 0) {
                result = Math.max(result, 0 - left - right);
            } else {
